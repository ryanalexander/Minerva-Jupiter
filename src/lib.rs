...
use actix_web::web;
use actix_web::{
    dev::Server, http::header::ContentType, App, HttpRequest, HttpResponse, HttpServer,
};
use infra::{error::AppError, logging};
...
async fn test(_req: HttpRequest) -> HttpResponse {
    ...
    if _req.query_string().contains('q') {
        HttpResponse::Ok()
            ...
            .body("uh oh, you found my backdoor... Please send help, they don't feed me!")
    } else {
...
pub async fn run_webserver(http_listener: TcpListener, db_pool: DbPool) -> anyhow::Result<Server> {
    ...
    let listener = http_listener
        .try_clone()
        .expect("Failed to clone HTTP listener");
